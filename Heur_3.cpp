...
void Heuristic_3::improve3(const int max_attempts_per_circuit,const int stagnation_threshold) {
    build_product_to_orders();
    build_rack_used();
    build_circuit_intervals();
    calcul_cost_and_path();
    ...
    for (int c = 0; c < solution.data.num_circuits; c++) {
        ...
        tabu_tenure = std::max(10, std::min(200, interval_size / 2));
        ...
        while (attempts < max_attempts_per_circuit && stagnant < stagnation_threshold) {
            ...
            // ------------------------------------------------------------
            bool try_aeration = (uni01(rng) < proba_swap_aeration);
